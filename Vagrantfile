...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sed -i -e "s/^local\s*all\s*postgres\s*peer/local all postgres trust/g" /etc/postgresql/9.5/main/pg_hba.conf
    ...
    createdb -U postgres -O demo -E utf8 demo

    # Install Redis
    apt-get -y install redis-server
    sed -i -e "s/^bind.*/bind 0.0.0.0/g" /etc/redis/redis.conf
